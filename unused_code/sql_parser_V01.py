...
class SQLQueryAnalyzer:
    ...
    def _parse_create_table_query(self, query):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        table_info = {}
        table_name_match = re.search(r"CREATE\s+TABLE\s+`?(\w+)`?\.`?(\w+)`?", query, re.IGNORECASE)
        if table_name_match:
            table_info["schema"] = table_name_match.group(1)
        ...
        column_definitions = re.findall(r"`(\w+)`\s+([\w()]+(\s+NOT NULL)?)", query, re.IGNORECASE)
        for col in column_definitions:
            columns.append({"name": col[0], "type": col[1].strip()})
    ...
    def get_parsed_data(self):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        return self.parsed_data[0].get("query")


class TableInfoExtractor:
    def __init__(self, create_table_query):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        logging.info("=== Parâmetros recebidos ===")
        logging.info(f"==> VAR: create_table_query TYPE: {type(create_table_query)}, CONTENT: {create_table_query}")
    ...
    def _extract_table_name(self):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        create_table_pattern = r"CREATE\s+TABLE\s+(IF\s+NOT\s+EXISTS\s+)?"
        match = re.search(create_table_pattern + r"`?(\w+)`?\.`?(\w+)`?", self.create_table_query, re.IGNORECASE)
        if match:
            self.database_name = match.group(2)
            self.table_name = match.group(3)
        else:
            match = re.search(create_table_pattern + r"`?(\w+)`?", self.create_table_query, re.IGNORECASE)
            if match:
                self.table_name = match.group(2)

    def _extract_columns(self):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        fields_section = re.search(r"\((.*)\)", self.create_table_query, re.DOTALL)
        if fields_section:
            fields = fields_section.group(1).split(',')
            for field in fields:
                field = field.strip()
                if field.upper().startswith('PRIMARY KEY'):
                    continue
                column_match = re.match(r"`(\w+)`\s+([\w()]+)(.*)", field)
                if column_match:
    ...
    def _extract_primary_keys(self):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        primary_key_match = re.search(r"PRIMARY KEY\s*\(([^)]+)\)", self.create_table_query, re.IGNORECASE)
        if primary_key_match:
    ...
    def _extract_indexes(self):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        index_matches = re.findall(r"INDEX\s*`?(\w*)`?\s*\(([^)]+)\)", self.create_table_query, re.IGNORECASE)
    ...
    def _extract_unique_keys(self):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        unique_key_matches = re.findall(r"UNIQUE\s*INDEX\s*`?(\w*)`?\s*\(([^)]+)\)", self.create_table_query, re.IGNORECASE)
        for unique_key in unique_key_matches:
    ...
    def _extract_foreign_keys(self):
        logging.info("=== Função: %s ===" % (sys._getframe().f_code.co_name))
        foreign_key_matches = re.findall(r"FOREIGN KEY\s*\(([^)]+)\)\s*REFERENCES\s*`?(\w+)`?\.`?(\w+)`?\s*\(([^)]+)\)\s*(ON DELETE\s+(\w+))?\s*(ON UPDATE\s+(\w+))?", self.create_table_query, re.IGNORECASE)
        for fk in foreign_key_matches:
...
def parse_sql_file(file_path:str) -> list:
    ...
    for query in queries:
        ...
        if not query_analyzer_data:
            ...
            continue
        if not isinstance(query_analyzer_data, str):
            logging.error("query_analyzer_data is not string")
            raise ValueError("query_analyzer_data is not string")

        if not "CREATE TABLE" in query_analyzer_data.upper():
            logging.info("Query is not a CREATE TABLE query")
            continue

        extractor = TableInfoExtractor(query_analyzer_data)
